...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set())  # A set of a_nodes - empty to start
        for action in self.all_actions: #for each action
            a_node = PgNode_a(action) #create a PgNode_a
            if a_node.prenodes.issubset(self.s_levels[level]): #does the proposed PgNode_a has prenodes that are a subset of the previous S level?
                self.a_levels[level].add(a_node) #add the action node
                for s_node in self.s_levels[level]:
                    a_node.parents.add(s_node) #connect S node as parent of the A node
                    s_node.children.add(a_node) #connect action node to the parent S node instance
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set()) # S set of s_nodes - empty to start
        for a_node in self.a_levels[level-1]: #for each A node in the previous level
            for effnode in a_node.effnodes: #for each S node representing the effect produced by the action
                self.s_levels[level].add(effnode) #add effect to the S level
                effnode.parents.add(a_node) #add A node to the parent set of the S node
                a_node.children.add(effnode) #add effnode as child of the a_node that created it
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(set(node_a1.action.effect_add).intersection(set(node_a2.action.precond_neg)) or
                    set(node_a1.action.effect_rem).intersection(set(node_a2.action.precond_pos)) or
                    set(node_a2.action.effect_add).intersection(set(node_a1.action.precond_neg)) or
                    set(node_a2.action.effect_rem).intersection(set(node_a1.action.precond_pos)))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_node_a1 in node_a1.parents: #for each state that is parent of node_a1
            for precond_node_a2 in node_a2.parents: #for each state that is parent of node_a2
                if precond_node_a1.is_mutex(precond_node_a2): #if is mutex
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        #Check if one state is the negation of the other (node_s1.is_pos != node_s2.is_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precond_node_s1 in node_s1.parents: #for each action that is parent of node_s1
            for precond_node_s2 in node_s2.parents: #for each action that is parent of node_s2
                if not precond_node_s1.is_mutex(precond_node_s2): #if the actions are not mutex
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal: #for each goal in the problem
            ...
            for level in range(len(self.s_levels)): #for each level
                if s_goal in self.s_levels[level]: #if the state is a goal
                    level_sum += level
                    break
        return level_sum
